...
## Setup quirks
- **No `.env.example`** — `.env` is gitignored; copy from another source or create fresh
...
./docker.sh
```
- `.env` is **gitignored** — create manually on VPS after pull:
  ```
  APP_NAME=EraTrans
  APP_ENV=production
  APP_DEBUG=false
  APP_KEY=                    # auto-generated by docker.sh
  APP_URL=https://tubes-capstone.my.id
  DB_CONNECTION=mysql
  DB_HOST=db                  # docker service name, not 127.0.0.1
  DB_PORT=3306
  DB_DATABASE=eratrans
  DB_USERNAME=laravel
  DB_PASSWORD=secret
  ```
